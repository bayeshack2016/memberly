...
const FetchAlipayConfig = async (uid) => {
  const alipay = await Alipay.findOne({ uid });
...
class AlipayCtl {
  ...
  async handleAliPayRefund(ctx) {
    ...
    const alipayConfig = await FetchAlipayConfig(order.uid);
  ...
  async fetchAlipay(ctx) {
    ...
    ctx.body = await Alipay.findOne({ uid: ctx.request.body.uid });
  }

  async fetchAlipayQrcode(ctx) {
    const alipayConfig = await FetchAlipayConfig(ctx.request.body.uid);
    var alipay_f2f = new alipayf2f(alipayConfig);
    const result = await alipay_f2f.createQRPay({
      tradeNo: `${ctx.request.body.orderId}`,
      subject: `${ctx.request.body.productName}${ctx.request.body.levelName}`,
      totalAmount: ctx.request.body.price,
      body: `购买${ctx.request.body.productName}${ctx.request.body.levelName}共${ctx.request.body.price}元`,
      timeExpress: 5,
    });
    if (!result) {
  ...
  async handleAlipayCallback(ctx) {
    ...
    const alipayConfig = await FetchAlipayConfig(orderInfo.uid);
    var alipay_f2f = new alipayf2f(alipayConfig);
    ...
    if (productType === 3) {
      ...
      await Customer.updateOne(
        { email: order.email, uid: order.uid },
        {
